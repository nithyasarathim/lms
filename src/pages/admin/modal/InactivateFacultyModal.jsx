...
const InactivateFacultyModal = ({
  ...
  const isDeactivating = faculty.isActive;
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[110] p-4 font-['Poppins']">
      <div className="bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-200">
        <div
          className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 mx-auto ${isDeactivating ? "bg-red-500" : "bg-green-500"} text-white shadow-lg shadow-blue-900/20`}
        >
          <Power size={20} />
        </div>

        <h3 className="text-xl font-bold text-[#08384F] text-center mb-2">
          {isDeactivating ? "Deactivate Faculty?" : "Activate Faculty?"}
        </h3>
        ...
        <p className="text-xs text-gray-500 text-center mb-8 leading-relaxed">
          ...
          ? This will {isDeactivating ? "restrict" : "restore"} their access to
          the platform.
        </p>
