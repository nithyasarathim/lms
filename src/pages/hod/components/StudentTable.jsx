...
import { getAllStudents } from "../api/hod.api";
...
const StudentTable = ({ academicYearId }) => {
  ...
  const exportToExcel = () => {
    ...
    const deptCode = filteredStudents[0]?.department?.code || "DEPT";
    const academicYearName = filteredStudents[0]?.academicYear?.name || "YEAR";
    const selectedYearFilter = filterYear ? `${filterYear}Year` : "AllYears";
    ...
    XLSX.writeFile(
      wb,
      `${deptCode}_${selectedYearFilter}_${academicYearName}.xlsx`,
    );
  };

  const handlePrint = () => {
    if (filteredStudents.length === 0) return;
    const deptName = filteredStudents[0]?.department?.name || "N/A";
      ...
      <html>
        ...
        <body>
          ...
          <div class="filter-info">
            <div><span class="filter-label">Department:</span> ${deptName}</div>
                    ...
                    <td className="px-8 py-2 text-right print:hidden">
                      <div className="flex gap-1 justify-end">
                        <button
                          ...
                          className="p-2 text-blue-500 flex items-center gap-3 text-xs font-semibold bg-blue-50 rounded-lg"
                        >
                          <Eye size={16} /> View
                        </button>
                      </div>
